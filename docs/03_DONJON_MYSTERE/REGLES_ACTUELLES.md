...
- Déplacement sur la grille avec ZQSD, WASD ou les flèches. Avancer contre un ennemi déclenche une attaque de base.
...
- Kaito : zone sur les ennemis à distance de Manhattan de 2 cases maximum, sans traverser les obstacles.
...
- Les escaliers du dernier étage exigent la mort du boss. Changer d’étage conserve le héros et son équipement, sa recharge et ses invocations ; le soin existant de 18 PV reste appliqué.
...
- Estimation initiale de durée d’une exploration complète : 9–15 minutes pour un héros autour de 25 dégâts/action, à confirmer en parties humaines. Les personnages puissants peuvent terminer plus vite. Voir `EQUILIBRAGE_RECOMPENSES.md` pour les limites et hypothèses.
...
- `node tools/dungeon-smoke.js --rewards-audit` : 89 vérifications puis estimation sur 200 cartes pour chacun des six paliers. Export dans `output/dungeon-rewards-audit.json`.
